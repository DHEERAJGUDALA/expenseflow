...
export function ManagerQueuePage() {
  ...
  const loadQueue = async () => {
    setIsLoading(true);
    try {
      const data = await managerApi.getQueue();
    ...
    <DashboardLayout>
      <div className="p-6 lg:p-8 max-w-6xl">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="page-header">
          ...
          <p className="page-subtitle">{expenses.length} expense{expenses.length !== 1 ? "s" : ""} waiting for your approval</p>
        </motion.div>
                    ...
                    <tr key={exp.id}>
                      ...
                      <td>
                        ...
                        {exp.converted_amount && exp.currency !== exp.company_currency && (
                          <p className="text-xs" style={{ color: "var(--text-muted)" }}>{formatCurrency(exp.amount, exp.currency)}</p>
                        )}
                      </td>
                      ...
                      <td>
                        <span className="badge badge-warning">
                          <Clock size={10} />
                          {getWaitingTime(exp.created_at)}
                        </span>
                      </td>
                      <td>
                        <div className="flex items-center justify-end gap-2">
                          <button onClick={() => openModal(exp, "approve")} className="btn btn-success btn-sm">
                            <CheckCircle size={14} /> Approve
                          </button>
                          <button onClick={() => openModal(exp, "reject")} className="btn btn-danger btn-sm">
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <p className="modal-description">
                      {modal.expense?.description} — {formatCurrency(modal.expense?.converted_amount || modal.expense?.amount, modal.expense?.company_currency || modal.expense?.currency)}
                    </p>
                  </div>
                ...
                <div className="mb-5">
                  ...
                  <textarea
                    ...
                    placeholder={modal.action === "reject" ? "Explain why this expense is being rejected..." : "Add a comment..."}
                    rows={3}
                    className="input resize-none"
                  />
                  {modal.action === "reject" && (
                    <p className="text-xs mt-1" style={{ color: comment.trim().length < 20 ? "var(--danger)" : "var(--success)" }}>
                ...
                <div className="flex justify-end gap-3">
                  ...
                  <button
                    ...
                    disabled={isProcessing || (modal.action === "reject" && comment.trim().length < 20)}
